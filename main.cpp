...
void updatePhysics(world& w, ofstream& coords) {
    const vec3d gravity = {0, -9.81f, 0}; // Гравитация должна быть отрицательной по Y
    ...
    for(auto& obj : w.objects) {
        ...
        obj.trajectory.push_back(obj.pos);
         ...
         if(obj.trajectory.size() == obj.maxTrajectoryPoints && obj.trajWritten == false)
        {
          coords.open ("Координаты.txt");
...
while(!start) {
	...
	try {
       		...
       		// Обработка ошибки
       		obj.mass = 1.0f;
     		printw("Error: invalid argument, did you enter a number?");
  	   }
	printw("\nEnter Cd: \n");
	try {
	...
	float c;
	printw("Enter atmosphere density: ");
	try {
                obj.vel.z = stof(consoleIn());
		if((c = stof(consoleIn())) <  0.0f) {
        	w->atmDensity = 1.125f;
        	clear();
        	refresh();
        	}
...
int main()
{
    ...
    vector<button> mainM = {
        ...
        {BUTTON, 10, 9, .targetVar = &magn, "Magnitude", 0, 50, 1},
